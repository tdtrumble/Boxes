...
$response = Invoke-WebRequest -Uri "https://api.themoviedb.org/3/discover/movie?api_key=$apiKey&include_adult=false&include_video=false&language=en-US&page=1&primary_release_date.gte=$formattedDate&region=US&sort_by=popularity.desc&watch_region=US&with_original_language=en&with_watch_providers=15"
...
# API request for tv
$response = Invoke-WebRequest -Uri "https://api.themoviedb.org/3/discover/tv?api_key=$apiKey&first_air_date.gte=$formattedDate&include_adult=false&language=en-US&page=1&sort_by=popularity.desc&watch_region=ES&with_origin_country=US&with_watch_providers=15"
